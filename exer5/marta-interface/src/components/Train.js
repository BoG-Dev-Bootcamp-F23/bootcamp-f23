export default function Train(props) {
    ...
    const trainStatus = onTime === "Delayed" ? "train-item-delayed" : "train-item-on-time"
    ...
    return (
        <div className="train-item-container">
            ...
            <div className="train-item-station-description">
                <p className="train-item-og-destination">{normalCaseHelper(ogStation)} &rarr; {normalCaseHelper(destination)}</p>
                <div className="train-item-line-delay">
                    <p className={`train-item-line-${currColor}`}>{currColor.charAt(0) + currColor.slice(1).toLowerCase()}</p>
            ...
            <div className={`train-item-arrival-time ${trainStatus}`}>
                ...
                {waitingTime === "Arriving" ? <p>{waitingTime}</p> : <p>{waitingTime.slice(2)}</p>}
            </div>
        </div>
    );
}
